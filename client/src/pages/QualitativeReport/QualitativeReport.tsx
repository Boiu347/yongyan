  ...
  return (
    <div className="p-8 pb-20">
      ...
      {project.overallSummary && (() => {
        const findings = project.overallSummary.split(/(?=\d+[)）])/).filter(s => s.trim().length > 5);
        return (
          <div className="mb-8 bg-gradient-to-br from-purple-50 via-indigo-50 to-blue-50 border border-purple-100 rounded-2xl p-8">
            <div className="flex items-center gap-2 mb-5">
              <Sparkles size={18} className="text-purple-500" />
              <h3 className="text-lg font-bold text-gray-900">整体研究发现</h3>
              <span className="text-[10px] px-2 py-0.5 bg-purple-100 text-purple-600 rounded-full">不分品牌</span>
            </div>
            <div className="space-y-4">
              {findings.map((finding, idx) => {
                const cleaned = finding.replace(/^\d+[)）]\s*/, '').trim();
                return (
                  <div key={idx} className="flex gap-4 items-start">
                    ...
                    <p className="text-sm text-gray-700 leading-relaxed pt-1">{cleaned}</p>
                  </div>
                );
              })}
            </div>
